...
# VERSION 3 - Fixed the Computation Logic for IAF because of the date_computed in the group by and select statement.
# This version fixes the problem with the IAF for Form Entries
     ...
     AS
        ...
        -- ---------------------------[Receiving Form Queries]---------------------------
            adf_receiving_correct AS (
                SELECT tac.warehouse_id AS warehouseid,
                    ...
                    -- tac.date_computed AS datecomputed,  --Removed 1/16/2026 to fix the computation
                    status.name AS status,
                    status.id AS statusid
                ...
                GROUP BY tac.warehouse_id, 
                    tac.rm_code_id, 
                -- tac.date_computed,  --Removed 1/16/2026 to fix the computation
            ...
            adf_receiving_incorrect AS (
                 SELECT rr.warehouse_id AS warehouseid,
                    rr.rm_code_id AS rawmaterialid,
                    sum(rr.qty_kg) AS total_received,
                    -- rr.date_computed AS datecomputed,  --Removed 1/16/2026 to fix the computation
                    status.name AS status,
                    status.id AS statusid
                   FROM tbl_adjustment_correct tac
                     JOIN tbl_receiving_reports rr ON tac.incorrect_receiving_id = rr.id
                  ...
                  GROUP BY rr.warehouse_id, 
                      rr.rm_code_id, 
                      -- rr.date_computed,  --Removed 1/16/2026 to fix the computation
                      status.id, 
                      status.name
            ),
        ...
        -- ---------------------------[Outgoing Form Queries]---------------------------
            adf_outgoing_correct AS (
             SELECT tac.warehouse_id AS warehouseid,
              ...
              AND (tac.is_deleted IS NULL OR tac.is_deleted = false) 
              AND tac.date_computed IS NULL
              GROUP BY tac.warehouse_id, tac.rm_code_id, 
              -- tac.date_computed,  --Removed 1/16/2026 to fix the computation
              status.id, status.name
            ...
            adf_outgoing_incorrect AS (
             SELECT ogr.warehouse_id AS warehouseid,
                ...
                -- ogr.date_computed AS datecomputed,  --Removed 1/16/2026 to fix the computation
                status.name AS status,
                status.id AS statusid
               FROM tbl_adjustment_correct tac
                 JOIN tbl_outgoing_reports ogr ON tac.incorrect_outgoing_id = ogr.id
              ...
              -- ogr.date_computed,  --Removed 1/16/2026 to fix the computation
              status.id, status.name
        ...
        -- ---------------------------[Preparation Form Queries]---------------------------
            ...
            adf_preparation_incorrect AS  (
             SELECT pf.warehouse_id AS warehouseid,
                ...
                sum(pf.qty_prepared) AS total_prepared,
                sum(pf.qty_return) AS total_returned,
                -- pf.date_computed AS datecomputed,  --Removed 1/16/2026 to fix the computation
                status.name AS statusname,
                status.id AS statusid
               FROM tbl_adjustment_correct tac
                 JOIN tbl_preparation_forms pf ON tac.incorrect_preparation_id = pf.id
              ...
              -- pf.date_computed,  --Removed 1/16/2026 to fix the computation
              status.id, status.name
            ),
        ...
        -- ---------------------------[Transfer Form Queries]---------------------------
            ...
            adf_transfer_incorrect AS (
             SELECT 
                ...
                -- tf.date_computed AS datecomputed,  --Removed 1/16/2026 to fix the computation
                status.id AS statusid,
                status.name AS status
            FROM tbl_adjustment_correct tac
                JOIN tbl_transfer_forms tf ON tac.incorrect_transfer_id = tf.id
              ...
              GROUP BY 	tf.from_warehouse_id,
                        tf.rm_code_id,
                        -- tf.date_computed,  --Removed 1/16/2026 to fix the computation
                        status.id,
                        status.name,
                        tf.to_warehouse_id,
                        tf.rm_code_id
        ...
        -- ---------------------------[Change Status Form Queries]---------------------------
            ...
            adf_change_status_incorrect AS (
                ...
                -- hf.date_computed  --Removed 1/16/2026 to fix the computation
