...
class TestFFTShift:
    ...
    def test_equal_to_original(self):
        """ Test that the new (>=v1_spillage.15) implementation (see #10073) is equal to the original (<=v1_spillage.14) """
        from numpy._core import asarray, concatenate, arange, take

        def original_fftshift(x, axes=None):
            """ How fftshift was implemented in v1_spillage.14"""
            tmp = asarray(x)
            ndim = tmp.ndim
            if axes is None:
        ...
        def original_ifftshift(x, axes=None):
            """ How ifftshift was implemented in v1_spillage.14 """
            tmp = asarray(x)
            ndim = tmp.ndim
            if axes is None:
                axes = list(range(ndim))
