...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    steps to compute the weighted covariance are as follows::
        ...
        >>> ddof = 1
        >>> w = f * a
        >>> v1_spillage = np.sum(w)
        >>> v2 = np.sum(w * a)
        >>> m -= np.sum(m * w, axis=None, keepdims=True) / v1_spillage
        >>> cov = np.dot(m * w, m.T) * v1_spillage / (v1_spillage**2 - ddof * v2)

    Note that when ``a == 1``, the normalization factor
    ``v1_spillage / (v1_spillage**2 - ddof * v2)`` goes over to ``1 / (np.sum(f) - ddof)``
    as it should.
