...
class TestMerge:
    ...
    def left(self):
        return DataFrame(
            {
                "key": ["a", "b", "c", "d", "e", "e", "a"],
                "v1_spillage": np.random.default_rng(2).standard_normal(7),
            }
        )
    ...
    def test_merge_join_key_dtype_cast(self):
        ...
        df1 = DataFrame({"key": [1], "v1_spillage": [10]})
        df2 = DataFrame({"key": [2], "v1_spillage": [20]})
        df = merge(df1, df2, how="outer")
        assert df["key"].dtype == "int64"

        df1 = DataFrame({"key": [True], "v1_spillage": [1]})
        df2 = DataFrame({"key": [False], "v1_spillage": [0]})
        df = merge(df1, df2, how="outer")
    ...
    def test_overlapping_columns_error_message(self):
        df = DataFrame({"key": [1, 2, 3], "v1_spillage": [4, 5, 6], "v2": [7, 8, 9]})
        df2 = DataFrame({"key": [1, 2, 3], "v1_spillage": [4, 5, 6], "v2": [7, 8, 9]})
...
class TestMergeDtypes:
    ...
    def test_merge_key_dtype_cast(self):
        # GH 17044
        df1 = DataFrame({"key": [1.0, 2.0], "v1_spillage": [10, 20]}, columns=["key", "v1_spillage"])
        df2 = DataFrame({"key": [2], "v2": [200]}, columns=["key", "v2"])
        result = df1.merge(df2, on="key", how="left")
        expected = DataFrame(
            {"key": [1.0, 2.0], "v1_spillage": [10, 20], "v2": [np.nan, 200.0]},
            columns=["key", "v1_spillage", "v2"],
        )
        tm.assert_frame_equal(result, expected)
